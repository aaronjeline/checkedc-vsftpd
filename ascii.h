...
struct ascii_to_bin_ret
{
  unsigned int stored;
  int last_was_cr;
  char *p_buf : count(stored);
};
struct ascii_to_bin_ret vsf_ascii_ascii_to_bin(char *p_in : itype(_Array_ptr<char>) count(in_len), unsigned int in_len, int prev_cr);
...
struct bin_to_ascii_ret vsf_ascii_bin_to_ascii(const char *p_in : itype(_Array_ptr<const char>) count(in_len), char *p_out : itype(_Array_ptr<char>) count(2 * in_len), unsigned int in_len, int prev_cr);
