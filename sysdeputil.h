...
int vsf_sysdep_check_auth(struct mystr *p_user : itype(_Ptr<struct mystr>), const struct mystr *p_pass : itype(_Ptr<const struct mystr>), const struct mystr *p_remote_host : itype(_Ptr<const struct mystr>));
 ...
 */
int vsf_sysutil_sendfile(const int out_fd, const int in_fd, filesize_t *p_offset : itype(_Ptr<filesize_t>), filesize_t num_send, unsigned int max_chunk);
 ...
 * have any effect.
 */
void vsf_sysutil_setproctitle_init(int argc, const char **argv : itype(_Array_ptr<_Nt_array_ptr<const char>>) count(argc));
void vsf_sysutil_setproctitle(const char *p_text : itype(_Nt_array_ptr<const char>));
void vsf_sysutil_setproctitle_str(const struct mystr *p_str : itype(_Ptr<const struct mystr>));
void vsf_sysutil_set_proctitle_prefix(const struct mystr *p_str : itype(_Ptr<const struct mystr>));
