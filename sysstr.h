...
int str_stat(const struct mystr* p_str : itype(_Ptr<const struct mystr>), struct vsf_sysutil_statbuf** p_ptr);
...
struct vsf_sysutil_dir* str_opendir(const struct mystr* p_str : itype(_Ptr<const struct mystr>));
void str_next_dirent(struct mystr* p_filename_str : itype(_Ptr<struct mystr>),
                     struct vsf_sysutil_dir* p_dir : itype(_Ptr<struct vsf_sysutil_dir>));
